...
.factory('Auth', ['AuthUser',function(AuthUser) {
	
	return {
	    allowed : function(permission) {
	        return $.inArray(permission, AuthUser.authorityValues);
	    }
	};}])
...
.directive('allowed', function(Auth){
    return {
        link : function(scope, elem, attr) {
            if(Auth.allowed(attr.allowed) == -1){
                elem.hide();
            }
        }
    };
...
.controller('contactsController',['$scope', '$http',function ($scope, $http) {
    ...
    $scope.url = "/BillingCrm/protected/contacts/";
